...
# ---- Config -----------------------------------------------------
MODEL_NAME = "Gemma4 31B (BIG, multimodal)"
MODEL_FILE = "gemma4-31b.sif"
MODEL_PATH = os.environ.get("MODEL_PATH", "/opt/models/gemma4-31b")

# Memory settings
MAX_CONTEXT_TOKENS = 100000  # Leave room for instructions + output (Model limit 256K)
OVERLAP_TOKENS = 500         # Overlap chunks to ensure no context is lost at boundaries

# ---- Progress logging -------------------------------------------

def log(msg):
    print(f"[gemma4] {msg}", file=sys.stderr)

def debug(msg):
    if args.debug:
        print(f"[gemma4][debug] {msg}", file=sys.stderr)

# ---- ARGUMENTS PARSING ------------------------------------------

def build_help_text():
    return f"""
Gemma4 Apptainer Runner
============================================

This container runs Gemma4 models with strict JSON-only output.
...
log("Loading model into GPU memory (with native SDPA)...")

# Load processor
processor = AutoProcessor.from_pretrained(MODEL_PATH, use_fast=False)

# Load model with native SDPA for memory optimization
