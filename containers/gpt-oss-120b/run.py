...
MODEL_FILE = "gpt-oss-120b.sif"
MODEL_PATH = os.environ.get("MODEL_PATH", "/opt/models/gpt-oss-120b")

# Memory settings
MAX_CONTEXT_TOKENS = 3000  # Leave room for instructions + output (Conservative estimate for large GPT model)
OVERLAP_TOKENS = 200        # Overlap chunks to ensure no context is lost at boundaries

# ---- Progress logging -------------------------------------------

def log(msg):
    print(f"[gpt-oss] {msg}", file=sys.stderr)

def debug(msg):
    if args.debug:
        print(f"[gpt-oss][debug] {msg}", file=sys.stderr)
...
log("Loading model into GPU memory (with Kernels Flash Attention 3)...")
